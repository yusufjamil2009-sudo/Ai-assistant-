...
class UstadCallInCallService : InCallService() {
    private val engine: CallAssistantEngine? get() = (application as? com.ustad.personalassistant.UstadApplication)?.callAssistantEngine
    override fun onCallAdded(call: Call) { super.onCallAdded(call); engine?.onCallAdded(call) }
    override fun onCallRemoved(call: Call) { engine?.finishCall(call); super.onCallRemoved(call) }
