...
import com.ustad.personalassistant.voice.VoiceSessionType
...
class CallAssistantEngine(private val context: Context, private val settings: SettingsRepository, private val aiBrain: AiBrain, private val voiceSessionManager: VoiceSessionManager) {
    ...
    fun onCallAdded(call: Call) {
        ...
        val id = UUID.randomUUID().toString(); val handle = call.details.handle?.schemeSpecificPart; activeCall = call; active = CallSession(id, handle, null, System.currentTimeMillis()); voiceSessionManager.setSessionType(VoiceSessionType.CALL_CONVERSATION_SESSION); _state.value = CallAssistantState.INCOMING_CALL; call.registerCallback(callback)
    ...
    fun finishCall(call: Call? = null) {
        ...
        timeoutRunnable?.let(handler::removeCallbacks); timeoutRunnable = null; activeCall?.unregisterCallback(callback); val session = active ?: return; session.endedAtMs = System.currentTimeMillis(); activeCall = null; active = null; voiceSessionManager.setSessionType(VoiceSessionType.NORMAL_ASSISTANT_SESSION); _state.value = CallAssistantState.CALL_ENDED
    ...
    fun destroy() { timeoutRunnable?.let(handler::removeCallbacks); timeoutRunnable = null; activeCall?.unregisterCallback(callback); activeCall = null; active = null; voiceSessionManager.setSessionType(VoiceSessionType.NORMAL_ASSISTANT_SESSION); scope.cancel(); _state.value = CallAssistantState.IDLE }
}
