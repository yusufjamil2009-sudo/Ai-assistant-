...
private fun UstadApp(viewModel: MainViewModel) {
    ...
    val app = LocalContext.current.applicationContext as UstadApplication
    ...
    MaterialTheme {
        Scaffold(bottomBar = {
            ...
            NavHost(navController, startDestination = "home", modifier = Modifier.padding(padding)) {
                ...
                composable("settings") { SettingsScreen(onAiProviders = { navController.navigate("ai-providers") }) }
...
private fun HomeScreen(state: AppState) {
    Column(Modifier.fillMaxSize().background(Color(0xFFF7FAFD)).verticalScroll(rememberScrollState()).padding(20.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Text("Permission and capability controls are centralized and refreshed from Android system state.")
        } }
        Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
...
private fun CapabilityCard(capability: Capability, title: String, status: CapabilityStatus, viewModel: MainViewModel, activity: MainActivity?) {
    Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
            Column(Modifier.weight(1f)) { Text(title, style = MaterialTheme.typography.titleMedium); Text(viewModel.explain(capability), style = MaterialTheme.typography.bodySmall) }
        ...
        if (status == CapabilityStatus.OFF || status == CapabilityStatus.ACTION_REQUIRED || status == CapabilityStatus.CONNECT || status == CapabilityStatus.NOT_ENROLLED) {
            Button(enabled = activity != null, onClick = { activity?.let { viewModel.request(it, capability) } }) { Text(when (status) { CapabilityStatus.CONNECT -> "CONNECT"; CapabilityStatus.ACTION_REQUIRED -> "OPEN SETTINGS"; CapabilityStatus.NOT_ENROLLED -> "SET UP"; else -> "ENABLE" }) }
        }
    } }
}
...
private fun StatusRow(label: String, status: CapabilityStatus) {
    val statusText = when (status) { CapabilityStatus.UNKNOWN -> "UNKNOWN"; CapabilityStatus.ON -> "ON"; CapabilityStatus.OFF -> "OFF"; CapabilityStatus.CONNECT -> "CONNECT"; CapabilityStatus.CONNECTED -> "CONNECTED"; CapabilityStatus.NOT_AVAILABLE -> "NOT AVAILABLE"; CapabilityStatus.ACTION_REQUIRED -> "ACTION REQUIRED"; CapabilityStatus.NOT_ENROLLED -> "NOT ENROLLED" }
    Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(5.dp)) { if (status == CapabilityStatus.ON || status == CapabilityStatus.CONNECTED) Icon(Icons.Outlined.CheckCircle, null); Text(statusText, style = MaterialTheme.typography.labelMedium) }
}

@Composable
private fun SettingsScreen(onAiProviders: () -> Unit) {
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        Text("Settings", style = MaterialTheme.typography.headlineMedium)
        Text("Centralized settings preserve the existing capability, security and automation boundaries.")
        HorizontalDivider()
        Button(onClick = onAiProviders, modifier = Modifier.fillMaxWidth()) { Text("AI PROVIDER MANAGER") }
        listOf("Assistant", "Voice", "AI Providers", "Speech-to-Text", "Text-to-Speech", "Permissions", "Security", "Protected Apps", "Notifications", "Calls", "Gmail", "Google Account", "Data", "Diagnostics", "About").forEach { Card(Modifier.fillMaxWidth()) { Text(it, Modifier.padding(16.dp), style = MaterialTheme.typography.titleMedium) } }
