    ...
    MaterialTheme {
        Scaffold(
            ...
            NavHost(
                ...
                composable("home") {
                    HomeScreen(state, app.voiceEngine, activity)
                }
                composable("permissions") {
                    PermissionCenterScreen(state, viewModel, activity)
                }
                composable("settings") {
                    SettingsScreen(
                        onAiProviders = { navController.navigate("ai-providers") },
                        onVoice = { navController.navigate("voice-settings") }
                    )
                }
                composable("ai-providers") {
                    AiProviderManagerScreen(app.aiProviderManager)
                }
                composable("voice-settings") {
                    VoiceSettingsScreen(
                        app.settingsRepository,
                        app.voiceProviderRegistry,
                        app.sttManager,
                        app.ttsManager
                    )
                }
...
private fun HomeScreen(state: AppState, voiceEngine: VoiceEngine, activity: MainActivity?) {
    ...
    val onVoiceEvent: (SttEvent) -> Unit = { event ->
        if (event.type == SttEventType.PARTIAL || event.type == SttEventType.FINAL) {
            transcript = event.result?.text.orEmpty()
        }
        voiceState = voiceEngine.getListeningState()
    }
    ...
    Column(
        ...
        Card(Modifier.fillMaxWidth()) {
            Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                Text("State: ${voiceEngine.getListeningState().name}")
                    ...
                    Button(
                        ...
                        onClick = {
                            ...
                            if (currentActivity != null) {
                                voiceEngine.startListening(currentActivity, onVoiceEvent)
                            }
                        }
                    ) {
    ...
    Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            if (
                ...
                status == CapabilityStatus.CONNECT ||
                status == CapabilityStatus.NOT_ENROLLED
            ) {
                Button(
                    enabled = activity != null,
                    onClick = {
                        val currentActivity = activity
                        if (currentActivity != null) viewModel.request(currentActivity, capability)
                    }
                ) {
                    Text(
                        when (status) {
...
private fun StatusRow(label: String, status: CapabilityStatus) {
    ...
    Row(
        ...
        if (status == CapabilityStatus.ON || status == CapabilityStatus.CONNECTED) {
            Icon(Icons.Outlined.CheckCircle, contentDescription = null)
        }
        Text("$label: $statusText", style = MaterialTheme.typography.labelMedium)
    }
}
...
private fun SettingsScreen(onAiProviders: () -> Unit, onVoice: () -> Unit) {
    Column(
        ...
        HorizontalDivider()
        Button(onClick = onAiProviders, modifier = Modifier.fillMaxWidth()) {
            Text("AI PROVIDER MANAGER")
        }
        Button(onClick = onVoice, modifier = Modifier.fillMaxWidth()) {
            Text("VOICE SETTINGS")
        }
        listOf(
            "Assistant", "Voice", "AI Providers", "Speech-to-Text", "Text-to-Speech",
            "Permissions", "Security", "Protected Apps", "Notifications", "Calls",
            "Gmail", "Google Account", "Data", "Diagnostics", "About"
        ).forEach {
            Card(Modifier.fillMaxWidth()) {
