...
data class MessageIntent(
    val type: com.ustad.personalassistant.ai.IntentType,
    val platform: MessagingPlatform? = null,
...
class MessagingIntentParser {
    fun parse(text: String): MessageIntent {
        ...
        val platform = when { lower.contains("whatsapp") -> MessagingPlatform.WHATSAPP; lower.contains("messenger") -> MessagingPlatform.MESSENGER; lower.contains("sms") || lower.contains("text message") -> MessagingPlatform.SMS; else -> null }
        ...
        val contact = Regex("(?i)(?:to|ko|for)\\s+([a-z][a-z0-9 _-]{1,40}?)(?=\\s+(?:on|par|via)\\b|\\s+(?:message|bolo|batao|bhejo)\\b|$)").find(t)?.groupValues?.getOrNull(1)?.trim()
