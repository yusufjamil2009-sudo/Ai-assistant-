...
enum class VoiceErrorCode {
    MIC_PERMISSION_REQUIRED, MIC_PERMISSION_DENIED, MICROPHONE_UNAVAILABLE,
    STT_PROVIDER_UNAVAILABLE, STT_TIMEOUT, STT_NETWORK_ERROR, STT_RATE_LIMITED,
    STT_AUTH_ERROR, STT_INVALID_RESPONSE, STT_ALL_PROVIDERS_FAILED,
    TTS_PROVIDER_UNAVAILABLE, TTS_TIMEOUT, TTS_NETWORK_ERROR, TTS_AUTH_ERROR,
    TTS_INVALID_RESPONSE, TTS_ALL_PROVIDERS_FAILED, NO_LOCAL_VOICE,
    AUDIO_FOCUS_INTERRUPTED, VOICE_SESSION_ERROR
}
...
data class SpeechToTextConfig(
    ...
    val supportedLanguages: Set<VoiceLanguage> = setOf(VoiceLanguage.HINDI, VoiceLanguage.HINGLISH, VoiceLanguage.ENGLISH, VoiceLanguage.MIXED),
...
interface SpeechToTextProvider {
    val config: SpeechToTextConfig
    fun start(language: VoiceLanguage, listener: (SttEvent) -> Unit): Result<Unit>
    fun streamAudio(audio: ByteArray): Result<Unit>
    fun stop(): Result<SttResult?>
    fun transcribe(audio: ByteArray, language: VoiceLanguage): Result<SttResult>
    fun isAvailable(): Boolean
    fun healthCheck(): VoiceProviderHealth
}

interface TextToSpeechProvider {
    val config: TextToSpeechConfig
...
interface VoiceAuthenticationManager {
    enum class Result { AUTHORIZED, UNAUTHORIZED, NOT_ENROLLED, UNAVAILABLE }
    fun status(): Result
}

class UnavailableVoiceAuthenticationManager : VoiceAuthenticationManager {
    override fun status() = VoiceAuthenticationManager.Result.UNAVAILABLE
}

interface VoiceInputNormalizer {
    fun normalize(text: String): String
}

class DefaultVoiceInputNormalizer : VoiceInputNormalizer {
...
object DefaultVoiceProviderSlots {
    fun stt(): List<SpeechToTextConfig> = listOf(
        ...
        SpeechToTextConfig("android_local", "Android / Local", enabled = true, priority = 99, offlineSupport = true)
    )
    fun tts(): List<TextToSpeechConfig> = listOf(
