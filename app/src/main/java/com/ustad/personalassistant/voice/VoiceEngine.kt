...
class AndroidVoiceEngine(private val permissionManager: PermissionManager, private val capabilityEngine: CapabilityEngine, private val sttManager: SpeechToTextManager, private val ttsManager: TextToSpeechManager, private val aiOrchestrator: AiAutomationOrchestrator, private val settings: SettingsRepository, private val normalizer: VoiceInputNormalizer = DefaultVoiceInputNormalizer(), private val actionGate: VoiceActionGate = VoiceActionGate()) : VoiceEngine {
    ...
    override fun startListening(activity: Activity, onEvent: (SttEvent) -> Unit) {
        ...
        if (permissionManager.verifyPermission(Capability.MICROPHONE) != CapabilityStatus.ON) {
    ...
    private fun beginListening() {
        ...
        operationJob = scope.launch {
            ...
            val result = sttManager.start(language) { event -> when (event.type) { SttEventType.PARTIAL -> callback(event); SttEventType.FINAL -> { sessionState = VoiceSessionState.PROCESSING; callback(event); processFinal(event.result?.text, event.result?.language ?: language) }; SttEventType.ERROR -> { sessionState = VoiceSessionState.ERROR; callback(event) } } }
    ...
    private fun processFinal(rawText: String?, detectedLanguage: VoiceLanguage) {
        ...
        val origin = if (mode == VoiceOperationMode.CALL_CONVERSATION_MODE) VoiceInputOrigin.CALLER else VoiceInputOrigin.AUTHORIZED_USER
        if (!actionGate.mayEnterActionPipeline(origin, true)) { sessionState = VoiceSessionState.IDLE; return }
