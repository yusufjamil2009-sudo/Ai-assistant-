...
class SpeechToTextManager(private val providers: () -> List<SpeechToTextProvider>, private val networkAvailable: () -> Boolean, private val cooldownMs: Long = 15_000L) {
    ...
    fun start(language: VoiceLanguage, listener: (SttEvent) -> Unit): Result<String> {
        ...
        return selected.start(language) { event ->
            if (event.type == SttEventType.FINAL) { failures[selected.config.providerId] = 0; lastSuccess[selected.config.providerId] = System.currentTimeMillis() }
            if (event.type == SttEventType.ERROR) recordFailure(selected.config.providerId)
            listener(event)
    ...
    fun transcribe(audio: ByteArray, language: VoiceLanguage): Result<SttResult> {
        ...
        for (provider in candidates(language)) {
            repeat(provider.config.retryCount.coerceIn(0, 3) + 1) { attempt ->
                ...
                val result = provider.transcribe(audio, language)
                if (result.isSuccess) { failures[provider.config.providerId] = 0; lastSuccess[provider.config.providerId] = System.currentTimeMillis(); return result }
                last = result.exceptionOrNull()
            }
            recordFailure(provider.config.providerId)
        }
        return Result.failure(last as? VoiceException ?: VoiceException(VoiceErrorCode.STT_ALL_PROVIDERS_FAILED, "All speech recognition providers failed."))
    }
    fun streamAudio(audio: ByteArray): Result<Unit> {
        if (audio.isEmpty()) return Result.success(Unit)
        val provider = candidates(VoiceLanguage.MIXED).firstOrNull { it.config.streamingSupport }
