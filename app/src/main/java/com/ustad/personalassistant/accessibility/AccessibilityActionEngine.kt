...
class AndroidAccessibilityActionEngine(private val serviceProvider: () -> AdvancedAccessibilityService?, private val securityManager: SecurityManager) : AccessibilityActionEngine {
    ...
    override fun findNodes(text: String) = withService { service -> val nodes = service.nodesByText(text); if (nodes.isEmpty()) failure(AccessibilityActionStatus.NODE_NOT_FOUND) else success(nodes) }
    override fun findByText(text: String) = findNode(text)
    override fun findByContentDescription(text: String) = withService { service -> service.nodeByContentDescription(text)?.let(::success) ?: failure(AccessibilityActionStatus.NODE_NOT_FOUND) }
    override fun findByViewId(viewId: String) = withService { service -> service.nodeByViewId(viewId)?.let(::success) ?: failure(AccessibilityActionStatus.NODE_NOT_FOUND) }
