...
enum class IntentType { CHAT, OPEN_APP, OPEN_MESSAGING_APP, READ_MESSAGES, READ_NOTIFICATION, SUMMARIZE_MESSAGES, COMPOSE_MESSAGE, SEND_MESSAGE, REPLY_MESSAGE, CANCEL_MESSAGE, READ_EMAIL, SEARCH_EMAIL, SUMMARIZE_EMAIL, DRAFT_EMAIL, SEND_EMAIL, CALL_CONTACT, DEVICE_DIAGNOSTICS, SETTINGS, SEARCH, SUMMARIZE, UNKNOWN }
enum class EntityType { APP_NAME, PLATFORM, CONTACT_NAME, RECIPIENT, MESSAGE_TEXT, CONVERSATION, MESSAGE_ID, ATTACHMENT_TYPE, EMAIL_ADDRESS, EMAIL_SUBJECT, SEARCH_QUERY, LOCATION, DATE, TIME }
...
data class ConversationContext(val recentMessages: List<String> = emptyList(), val currentTask: String? = null, val currentAction: String? = null, val currentApp: String? = null, val previousIntent: IntentType? = null, val entities: List<AiEntity> = emptyList(), val maxMessages: Int = 12) { fun bounded(): ConversationContext = copy(recentMessages = recentMessages.takeLast(maxMessages.coerceIn(1, 50))) }
