...
class AiProviderManager(context: Context) {
    ...
    fun providers(): List<AiProvider> = allConfigs().map { config ->
        HttpAiProvider(config, apiKey = { secrets.get("api_key_${config.providerId}") })
    }

    fun buildApiManager(networkState: () -> NetworkState = { NetworkState.ONLINE }): ApiManager = ApiManager(providers(), networkState)

    private fun load() {
        val raw = prefs.getString("configs", null) ?: return
        runCatching {
    ...
    private fun persist() {
        ...
        configs.values.forEach { c ->
            array.put(JSONObject().apply {
                put("providerId", c.providerId); put("displayName", c.displayName); put("enabled", c.enabled); put("priority", c.priority); put("model", c.model); put("endpoint", c.endpoint ?: ""); put("timeoutMs", c.timeoutMs); put("retryCount", c.retryCount)
                put("capabilities", JSONArray(c.capabilities.map { it.name }))
            })
        }
