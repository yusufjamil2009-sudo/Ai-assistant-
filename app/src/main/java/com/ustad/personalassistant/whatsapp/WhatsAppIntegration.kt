...
class AndroidWhatsAppIntegration : WhatsAppIntegration {
    override fun openWhatsApp(context: Context): Boolean {
        return try { val intent = context.packageManager.getLaunchIntentForPackage("com.whatsapp") ?: return false; context.startActivity(intent); true } catch (_: Exception) { false }
    }
